...
Public Class frmExportBookingMenu
    ...
    Public Sub NewRecord() Implements ICommonFunction.NewRecord
        If MsgBox("Create new record?", MsgBoxStyle.YesNo + MsgBoxStyle.Question, "System Message") = MsgBoxResult.Yes Then
            clsExportRecord = New clsExportBookingHeader
            ClearUserInput()
    ...
    Public Sub ResetRecord() Implements ICommonFunction.ResetRecord
        ...
        clsComboBox = New clsPopulateComboBox(cboFinalDestCountry, "SELECT * FROM lib_country ORDER BY `Description`", "Description", "PK")
        clsComboBox.PopComboBox()
                    ...
                    With clsExportRecord
                        ...
                        For Each dtgRow As DataGridViewRow In dtgServices.Rows
                            If dtgRow.Cells(colSSelected.Name).Value = True Then
                                clsServices = New clsExportBookingServices
                        ...
                        For Each dtgRow As DataGridViewRow In dtgDocuments.Rows
                            If dtgRow.Cells(colDSelected.Name).Value = True Then
                                clsDocs = New clsExportBookingDocuments
...
lnAccountType:
    ...
    Public Sub PopulateUserInput(ByVal clsTempExportRecord As clsExportBookingHeader)
        If Not IsNothing(clsTempExportRecord) And clsTempExportRecord._ID > 0 Then
            ...
            With clsTempExportRecord
                ...
                cboFinalDestCountry.SelectedValue = ._FinalDestinationDetails._CountryDetails._PK
    ...
    Public Sub PopulateMasterBooking(ByVal clsTemp As clsExportMasterBooking)
        ClearMasterBooking()
    ...
    Private Sub PopulateRequiredDocuments()
        Try
            ...
            If cnnDBMaster.State <> ConnectionState.Open Then cnnDBMaster.Open()
            cmdSQL.Connection = cnnDBMaster
            cmdSQL.CommandText = "SELECT * FROM lib_booking_documents WHERE FreightType = 2 AND Active = TRUE"
    ...
    Private Function CheckRequiredEntries() As Boolean
        ...
        If cboFinalDestCountry.SelectedIndex = -1 Then
            MsgBox("Please select Final Destination.", MsgBoxStyle.Exclamation, "System Message")
            CheckRequiredEntries = False
            cboFinalDestCountry.Focus()
            Exit Function
        End If
    ...
    Private Sub ClearUserInput()
        ...
        cboFinalDestCountry.SelectedIndex = -1
    ...
    Public Sub ChangeEnabledButtons(ByVal blNew As Boolean, ByVal blEdit As Boolean, ByVal blSave As Boolean, ByVal blReset As Boolean,
        ...
        cboFinalDestCountry.Enabled = blUserInput
    ...
    Private Sub frmExportBookingMenu_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ResetRecord()
    End Sub

    Private Sub frmExportBookingMenu_Activated(sender As Object, e As EventArgs) Handles Me.Activated
        EnableButtons.UpdateEnabledButtons()
    End Sub

    Private Sub frmExportBookingMenu_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
    ...
    Private Sub lblStatusDetails_Click(sender As Object, e As EventArgs) Handles lblStatusDetails.Click
        If clsExportRecord._ID > 0 Then
            MsgBox("Status: " & txtStatus.Text.ToUpper & vbNewLine &
               "Status Update By: " & clsExportRecord._StatusByDetails._First_Name & " " & clsExportRecord._StatusByDetails._Last_Name & vbNewLine &
               "Status Date: " & clsExportRecord._StatusDate, MsgBoxStyle.Information, "Status Details")
        End If
    End Sub
    ...
    Private Sub cmdSelectConsignor_Click(sender As Object, e As EventArgs) Handles cmdSelectConsignor.Click
        With frmSelectClient
            ...
            clsExportRecord._AccountTypeDetails._Prefix = .clsSelectedClient._Account_TypePrefix
    ...
    Private Sub cboFinalDestCountry_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cboFinalDestCountry.SelectedIndexChanged
        Dim objSelectedValue As IConvertible = TryCast(Me.cboFinalDestCountry.SelectedValue, IConvertible)

        If Len(CInt(objSelectedValue)) > 0 Then
            Dim clsCombo As New clsPopulateComboBox(cboFinalDestPort, "SELECT * FROM lib_port WHERE Country_PK = " & CInt(objSelectedValue) & " ORDER BY `Description`", "Description", "PK")
            clsCombo.PopComboBox()
        Else
            cboFinalDestPort.Items.Clear()
        End If
    End Sub
