...
Public Class frmBrokerageExportTariffFCL
    ...
    Public Sub SaveTariff()
        ...
        If Len(Me.txtCtrlNbr.Text) = 0 Then
            ...
            Try
                cmdSQL.CommandText = ("INSERT INTO tbl_brokerageexporttarifffcl (`CtrlNbr`, `ChargeCode`, `ChargeName`, `TypeID`, `TypeDesc`, `Price`, `EncodedBy`, `EncodedDate`, `ModifiedBy`, `ModifiedDate`) VALUES " &
                                                        "               (@CtrlNbr, @ChargeCode, @ChargeName, @TypeID, @TypeDesc, @Price, @EncodedBy, @EncodedDate, @ModifiedBy, @ModifiedDate)")


                With cmdSQL.Parameters
                    Dim ctrlnbr As String = "EBFCL" & GetCtrlNbr(strCompanyCode, strSiteCode, "BrokerageExportTariffFCL")
            ...
            UpdateCtrlNbrCounter(strCompanyCode, strSiteCode, "BrokerageExportTariffFCL")
            ...
            SaveTransLogs("Create New", Me.Name, "Brokerage Export Tariff FCL", Me.txtCtrlNbr.Text, strCurrentUser)

        Else
            'update record

            Dim sSQL As String = "UPDATE  tbl_brokerageexporttarifffcl Set "
            sSQL += " Price=@Price, "
            ...
            Me.lblModifiedBy.Text = strCurrentUser
            Me.lblDateModified.Text = Format(dtCurrent, "yyyy-MM-dd hh:mm:ss")
            SaveTransLogs("Edit Record", Me.Name, "Brokerage Export Tariff FCL", Me.txtCtrlNbr.Text, strCurrentUser)
        End If
    ...
    Public Sub PopTariff(ByVal dg As DataGridView)
        ...
        strSQL = "Select * " &
                 "FROM tbl_brokerageexporttarifffcl"
    ...
    Public Sub SearchTariff(ByVal str As String)
        ...
        strSQL = "Select * " &
                 "FROM tbl_brokerageexporttarifffcl " &
                 "WHERE tbl_brokerageexporttarifffcl.CtrlNbr = '" & str & "'"
    ...
    Private Sub frmBrokerageExportTariffFCL_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        FillComboBox(Me.cboChargeName, "Select ChargeName from lib_charges where B = 1 ORDER BY ChargeName", "ChargeName")
        FillComboBox(Me.cboType, "Select Description from lib_brokerageexporttarifffcl where Description is not null", "Description")
        ChangeEnabledButtons(True, True, False, False, True, True, True, True, True, False)
        PopTariff(Me.dgBrkgTariff)
    End Sub

    Private Sub frmBrokerageExportTariffFCL_Activated(sender As Object, e As EventArgs) Handles Me.Activated
    ...
    Private Sub cboType_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cboType.SelectedIndexChanged
        ...
        Me.txtType.Text = GetIntegerValue("Select PK from lib_brokerageexporttarifffcl where Description = '" & Me.cboType.Text & "'", "PK")
