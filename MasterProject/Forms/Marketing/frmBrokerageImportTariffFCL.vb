...
Public Class frmBrokerageImportTariffFCL
    ...
    Public Sub SaveTariff()
        ...
        If Len(Me.txtCtrlNbr.Text) = 0 Then
            ...
            Try
                cmdSQL.CommandText = ("INSERT INTO tbl_brokerageimporttarifffcl (`CtrlNbr`, `ChargeCode`, `ChargeName`, `TypeID`, `TypeDesc`, `Price`, `EncodedBy`, `EncodedDate`, `ModifiedBy`, `ModifiedDate`) VALUES " &
                                                        "               (@CtrlNbr, @ChargeCode, @ChargeName, @TypeID, @TypeDesc, @Price, @EncodedBy, @EncodedDate, @ModifiedBy, @ModifiedDate)")


                With cmdSQL.Parameters
                    Dim ctrlnbr As String = "IBFCL" & GetCtrlNbr(strCompanyCode, strSiteCode, "BrokerageImportTariffFCL")
            ...
            UpdateCtrlNbrCounter(strCompanyCode, strSiteCode, "BrokerageImportTariffFCL")
            ...
            SaveTransLogs("Create New", Me.Name, "Brokerage Import Tariff FCL", Me.txtCtrlNbr.Text, strCurrentUser)

        Else
            'update record

            Dim sSQL As String = "UPDATE  tbl_brokerageimporttarifffcl Set "
            sSQL += " Price=@Price, "
            ...
            Me.lblModifiedBy.Text = strCurrentUser
            Me.lblDateModified.Text = Format(dtCurrent, "yyyy-MM-dd hh:mm:ss")
            SaveTransLogs("Edit Record", Me.Name, "Brokerage Import Tariff FCL", Me.txtCtrlNbr.Text, strCurrentUser)
        End If
    ...
    Public Sub PopTariff(ByVal dg As DataGridView)
        ...
        strSQL = "Select * " &
                 "FROM tbl_brokerageimporttarifffcl"
    ...
    Public Sub SearchTariff(ByVal str As String)
        ...
        strSQL = "Select * " &
                 "FROM tbl_brokerageimporttarifffcl " &
                 "WHERE tbl_brokerageimporttarifffcl.CtrlNbr = '" & str & "'"
    ...
    End Sub
    Private Sub frmBrokerageTariffFCL_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        FillComboBox(Me.cboChargeName, "Select ChargeName from lib_charges where B = 1 ORDER BY ChargeName", "ChargeName")
        FillComboBox(Me.cboType, "Select Description from lib_brokerageimporttarifffcl where Description is not null", "Description")
        ChangeEnabledButtons(True, True, False, False, True, True, True, True, True, False)
        PopTariff(Me.dgBrkgTariff)
    End Sub

    Private Sub frmBrokerageTariffFCL_Activated(sender As Object, e As EventArgs) Handles Me.Activated
    ...
    Private Sub cboType_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cboType.SelectedIndexChanged
        ...
        Me.txtType.Text = GetIntegerValue("Select PK from lib_brokerageimporttarifffcl where Description = '" & Me.cboType.Text & "'", "PK")
    End Sub
