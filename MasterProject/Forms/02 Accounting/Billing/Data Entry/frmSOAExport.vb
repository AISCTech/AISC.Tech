...
Public Class frmSOAExport
    ...
    Private Sub frmSOAExport_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ClearEntries(False)
        ...
        'FillComboBox(Me.cboBilledTo, "Select Description from tbl_client INNER JOIN tbl_clienttype ON tbl_client.Code = tbl_clienttype.Client_Code where ParamsCode = 2", "Description")
    ...
    Public Sub SaveBilling()
        ...
        'SAVE bill_impsoa_main
        If Len(Me.lblRefNo.Text) = 0 Then
            'NEW BILLING
            Dim strRefNo As String = "BEXSO" & GetCtrlNbr(strCompanyCode, strSiteCode, "Billing_Export_SOA")
            Me.lblRefNo.Text = strRefNo
            Dim dtCurrent As Date = GetServerDate()
            strsql = "INSERT INTO bill_expsoa_main(RefNo, TransactionType,  SOANo, BookingNos,  Shipper, BilledTo, BilledToAddress, Attention, Remarks, " &
                        " ExRate, TotalSOA, TotalAmtCurrency," &
                        " TotalAmtPHP, TermsOfPayment, PrepBy, PrepDate, Posted, Cancel) VALUES(@RefNo, @TransactionType," &
                        " @SOANo, @BookingNos, @Shipper, @BilledTo, @BilledToAddress, @Attention, @Remarks, @ExRate," &
                        " @TotalSOA, @TotalAmtCurrency, @TotalAmtPHP, @TermsOfPayment, @PrepBy, @PrepDate," &
                        " @Posted, @Cancel)"
            ...
            With cmdSQL.Parameters
                ...
                .AddWithValue("@SOANo", Me.txtCISOANo.Text)
                ...
                .AddWithValue("@TotalSOA", CDbl(Me.txtTotalSOA.Text))
            ...
            UpdateCtrlNbrCounter(strCompanyCode, strSiteCode, "Billing_Export_SOA")

            'save to transaction logs
            SaveTransLogs("Create New Export Billing SOA", Me.Name, "Billing No.", Me.lblRefNo.Text, strCurrentUser)
        Else
            'EDIT BILLING
            strsql = "UPDATE bill_expsoa_main SET" &
            ...
            'save to transaction logs
            SaveTransLogs("Edit Export Billing SOA", Me.Name, "Billing No.", Me.lblRefNo.Text, strCurrentUser)
        End If

        'SAVE bill_expsoa_charges
        strsql = "DELETE FROM bill_expsoa_charges WHERE RefNo = '" & Me.lblRefNo.Text & "'"
        ...
        For intRow = 0 To Me.dtgServiceCharge.RowCount - 1
            If Len(Me.dtgServiceCharge.Rows(intRow).Cells("colSCharges").Value) > 0 Then
                strsql = "INSERT INTO bill_expsoa_charges(RefNo, Charges, Terms," &
        ...
        'SAVE bill_expsoa_acctg_entry
        strsql = "DELETE FROM bill_expsoa_acctg_entry WHERE RefNo = '" & Me.lblRefNo.Text & "'"
        ...
        DeleteToGLTable(Me.lblRefNo.Text, 15)

        Dim ctrRow As Integer

        For ctrRow = 0 To Me.dtgAcctgTitle.Rows.Count - 1
            With Me.dtgAcctgTitle.Rows(ctrRow)
                strsql = "INSERT INTO bill_expsoa_acctg_entry (RefNo, Branch, AcctCode, AcctTitle, SubAcctCode," &
                ...
                InsertToGLTable(15, GetGeneralLedgerModuleName(15), Me.lblRefNo.Text, .Cells(0).Value, .Cells(1).Value, .Cells(2).Value, .Cells(3).Value, CDbl(.Cells(9).Value), CDbl(.Cells(10).Value), .Cells(4).Value, .Cells(5).Value, .Cells(6).Value, .Cells(7).Value, .Cells(8).Value)
...
ByVal blReOpen As Boolean, ByVal blUserInput As Boolean)
        With EnableButtons
            ...
            Me.txtAttention.ReadOnly = blUserInput
            Me.txtRemarks.ReadOnly = blUserInput
        End With
    End Sub
    ...
    Public Sub ClearEntries(ByVal blNew As Boolean)
        ...
        If blNew = False Then
            Me.txtCISOANo.Text = ""
            'Me.cboBilledTo.SelectedIndex = -1
            Me.txtShipper.Text = ""
    ...
    Public Function SearchBooking(ByVal str As String) As Boolean
        ...
        Dim strsql As String = "SELECT v_exportbookingheader.*, lib_city.Description AS `CityDesc`, tbl_frwdg_docs_hbl.Freight AS `FreightTerms` FROM ((v_exportbookingheader INNER JOIN lib_city ON v_exportbookingheader.ConsignorAddrCity = lib_city.PK) LEFT JOIN tbl_frwdg_docs_bookings ON v_exportbookingheader.ID = tbl_frwdg_docs_bookings.BookingID) LEFT JOIN tbl_frwdg_docs_hbl ON tbl_frwdg_docs_bookings.FrwdgDocsID = tbl_frwdg_docs_hbl.ID  WHERE v_exportbookingheader.BookingNo = '" & str & "'"
        ...
        While reader.Read
            ...
            If IsDBNull(reader.Item("FreightTerms")) = True Then
                Me.txtFreightTerms.Text = "Not hbl"
            Else
                Select Case reader.Item("FreightTerms")
                    Case "P"
                        Me.txtFreightTerms.Text = "Prepaid"
                    Case "C"
                        Me.txtFreightTerms.Text = "Collect"
                    Case Else
                        Me.txtFreightTerms.Text = "Unassigned"
                End Select

            End If

            Me.txtBilledTo.Text = reader.Item("ConsignorName")
            Me.txtShprAddr.Text = reader.Item("ConsignorAddr1") & vbNewLine & reader.Item("CityDesc")
            ...
            Me.txtETD.Text = reader.Item("ETD")
    ...
    Public Sub PostRecord() Implements ICommonFunction.PostRecord
        ...
        If CDbl(Me.txtTotalSOA.Text) > 0 Then
            strSOA = GetRegisteredFormNo(2, 2)
            If strSOA = -1 Then
        ...
        Dim strsql As String = "UPDATE bill_expsoa_main SET" &
                               " SOANo = @SOANo, " &
                               " Posted = @Posted, " &
                               " PostedBy = @PostedBy, " &
        ...
        Try
            ...
            cmdSQL.CommandText = "UPDATE reg_trn_main SET " &
                                ...
                                "WHERE reg_group = 2 AND reg_module = 1 " &
                                    "AND reg_no = @reg_no"
            ...
            MsgBox("Billing Reference No. " & Me.lblRefNo.Text & " is Posted!" & vbCrLf &
                    "S.O.A.#: " & strSOA, MsgBoxStyle.Information, "System Message")
    ...
    Public Sub CancelRecord() Implements ICommonFunction.CancelRecord
        ...
        'cancel record
        If Me.tslblStatus2.Text <> "Open" Then
            MsgBox("Record already " & Me.tslblStatus2.Text & "!", vbInformation, "Cannot Cancel")
            Exit Sub
        End If
        ...
        If MsgBox("Are you sure you want cancel billing : " & Me.lblRefNo.Text, MsgBoxStyle.Question + MsgBoxStyle.YesNo, "System Message") = MsgBoxResult.No Then
            Exit Sub
        End If
        strsql = "UPDATE bill_expsoa_main SET Cancel=TRUE, CancelDate=@CancelDate, CancelBy='" & strCurrentUser & "' WHERE RefNo = '" & Me.lblRefNo.Text & "' AND Cancel = FALSE"
    ...
    Public Sub ReOpenRecord() Implements ICommonFunction.ReOpenRecord
        ...
        'unpost record
        If Me.tslblStatus2.Text = "Open" Then
            MsgBox("Record already " & Me.tslblStatus2.Text & "!", vbInformation, "System Message")
            Exit Sub
        End If
        ...
        If MsgBox("Are you sure you want to re-open billing : " & Me.lblRefNo.Text, MsgBoxStyle.Question + MsgBoxStyle.YesNo, "System Message") = MsgBoxResult.No Then
            Exit Sub
        End If
        strsql = "UPDATE bill_expsoa_main SET Cancel=FALSE, CancelDate=null, CancelBy=null, Posted=FALSE, PostedDate=null, PostedBy=null WHERE RefNo = '" & Me.lblRefNo.Text & "'"
    ...
    Private Sub SOAToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SOAToolStripMenuItem.Click
        ...
        SaveTransLogs("Print Export SOA Billing", Me.Name, "Billing No.", Me.lblRefNo.Text, strCurrentUser)
        frmPrintPreview.PrintExpSOA()
        frmPrintPreview.ShowDialog()
    End Sub

    Private Sub DraftSOAToolStripMenuItem_Click(sender As Object, e As EventArgs)
        If Len(Me.lblRefNo.Text) = 0 Then
            MsgBox("Nothing to Print!", vbInformation, "System Message")
            Exit Sub
        End If
    ...
    Public Sub ComputeTotals()
        ...
        Me.txtTotalSOA.Text = Format$(dblSOA, "n2")
        Me.txtTotalAmtPHP.Text = Format$((dblBill), "n2")
    End Sub
    Public Sub GetAccountDetails(ByVal dg As DataGridView)
        ...
        For i = 0 To dg.Rows.Count - 1
            ...
            If Len(h.Cells(1).Value) <> 0 Then
                h.Cells(2).Value = "S.O.A."
                h.Cells(10).Value = "2020"
                h.Cells(11).Value = "2"
                h.Cells(12).Value = "1150"
                h.Cells(13).Value = "2"
    ...
    Public Sub SearchBilling(ByVal strRefNo As String, ByVal strField As String)
        ...
        'bill_impsoa_main
        strsql = "SELECT * " &
                    "FROM bill_expsoa_main " &
                    "WHERE bill_expsoa_main.RefNo = '" & strRefNo & "'"
        ...
        While reader.Read
            ...
            Me.txtCISOANo.Text = IIf(IsDBNull(reader.Item("SOANO")) = False, reader.Item("SOANo"), "")
            ...
            Me.txtTotalSOA.Text = Format$(reader.Item("TotalSOA"), "n2")
        ...
        'bill_expsoa_charges
        strsql = "SELECT * FROM bill_expsoa_charges WHERE RefNo = '" & strRefNo & "'"
        ...
        'bill_exp_acctg_entry
        strsql = "SELECT * FROM bill_expsoa_acctg_entry WHERE RefNo = '" & strRefNo & "'"
    ...
    Private Sub DraftToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles DraftToolStripMenuItem.Click
        ...
        SaveTransLogs("Print Draft Export SOA Billing", Me.Name, "Billing No.", Me.lblRefNo.Text, strCurrentUser)
        frmPrintPreview.PrintExpSOA_D()
        frmPrintPreview.ShowDialog()
    End Sub

    Private Sub SoftCopySOAToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SoftCopySOAToolStripMenuItem.Click
        If Len(Me.lblRefNo.Text) = 0 Then
            MsgBox("Nothing to Print!", vbInformation, "System Message")
            Exit Sub
        End If
        ...
        If Me.tslblStatus2.Text <> "Posted" Then
            ...
            Exit Sub
        End If
        SaveTransLogs("Print Soft Copy Export SOA Billing", Me.Name, "Billing No.", Me.lblRefNo.Text, strCurrentUser)
        frmPrintPreview.PrintExpSOA_S()
        frmPrintPreview.ShowDialog()
    End Sub
End Class