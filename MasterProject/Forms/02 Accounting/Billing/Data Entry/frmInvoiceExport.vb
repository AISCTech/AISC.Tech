...
Public Class frmInvoiceExport
    ...
    Public Sub SaveBilling()
        ...
        If Len(Me.lblRefNo.Text) = 0 Then
            'NEW BILLING
            Dim strRefNo As String = "BEXCI" & GetCtrlNbr(strCompanyCode, strSiteCode, "Billing_Export_Invoice")
            ...
            UpdateCtrlNbrCounter(strCompanyCode, strSiteCode, "Billing_Export_Invoice")
        ...
        strsql = "DELETE FROM bill_expinv_charges WHERE RefNo = '" & Me.lblRefNo.Text & "'"
        ...
        DeleteToGLTable(Me.lblRefNo.Text, 4)
        ...
        For ctrRow = 0 To Me.dtgAcctgTitle.Rows.Count - 1
            With Me.dtgAcctgTitle.Rows(ctrRow)
                ...
                InsertToGLTable(4, GetGeneralLedgerModuleName(4), Me.lblRefNo.Text, .Cells(0).Value, .Cells(1).Value, .Cells(2).Value, .Cells(3).Value, CDbl(.Cells(9).Value), CDbl(.Cells(10).Value), .Cells(4).Value, .Cells(5).Value, .Cells(6).Value, .Cells(7).Value, .Cells(8).Value)
            End With
        Next
...
ByVal blReOpen As Boolean, ByVal blUserInput As Boolean)
    ...
    '        grid.Cells(strTermsCol).Value = "Charge Invoice"
    '        grid.Cells(strDRCol).Value = "4120"
    '        grid.Cells(strDRSubCol).Value = "1"
    '        grid.Cells(strCRCol).Value = "1110"
    '        grid.Cells(strCRSubCol).Value = "1"
    '        'grid.Cells(14).Value = reader.Item("ChargeCode")
    '    End While
    ...
    Private Sub ChargeInvoiceToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ChargeInvoiceToolStripMenuItem.Click
        ...
        SaveTransLogs("Print Export Invoice Billing", Me.Name, "Billing No.", Me.lblRefNo.Text, strCurrentUser)
        frmPrintPreview.PrintExpCI()
    ...
    Public Sub ReOpenRecord() Implements ICommonFunction.ReOpenRecord
        ...
        If MsgBox("Are you sure you want to re-open billing : " & Me.lblRefNo.Text, MsgBoxStyle.Question + MsgBoxStyle.YesNo, "System Message") = MsgBoxResult.No Then
            Exit Sub
        End If
        strsql = "UPDATE bill_expinv_main SET Cancel=FALSE, CancelDate=null, CancelBy=null, Posted=FALSE, PostedDate=null, PostedBy=null WHERE RefNo = '" & Me.lblRefNo.Text & "'"
    ...
    Private Sub DraftToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles DraftToolStripMenuItem.Click
        ...
        SaveTransLogs("Print Draft Export Invoice Billing", Me.Name, "Billing No.", Me.lblRefNo.Text, strCurrentUser)
        frmPrintPreview.PrintExpCI_D()
        frmPrintPreview.ShowDialog()
    End Sub
    ...
    Private Sub SoftCopyChargeInvoiceToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SoftCopyChargeInvoiceToolStripMenuItem.Click
        ...
        SaveTransLogs("Print Soft Copy Export Invoice Billing", Me.Name, "Billing No.", Me.lblRefNo.Text, strCurrentUser)
        frmPrintPreview.PrintExpCI_S()
        frmPrintPreview.ShowDialog()
    End Sub
End Class