...
Public Class frmSOA
    ...
    Public Sub SaveBilling()
        ...
        If Len(Me.lblRefNo.Text) = 0 Then
            'NEW BILLING
            Dim strRefNo As String = "BIBSO" & GetCtrlNbr(strCompanyCode, strSiteCode, "Billing_Import_Brokerage_Trucking_SOA")
            Me.lblRefNo.Text = strRefNo
            Dim dtCurrent As Date = GetServerDate()
            strsql = "INSERT INTO bill_impsoa_main(RefNo, TransactionType,  SOANo, BookingNos, Consignee, Shipper, BilledTo, BilledToAddress, " &
                        " ExRate, TotalSOA, TotalAmtCurrency," &
                        " TotalAmtPHP, TermsOfPayment, PrepBy, PrepDate, Posted, Cancel) VALUES(@RefNo, @TransactionType," &
                        " @SOANo, @BookingNos, @Consignee, @Shipper, @BilledTo, @BilledToAddress, @ExRate," &
            ...
            UpdateCtrlNbrCounter(strCompanyCode, strSiteCode, "Billing_Import_Brokerage_Trucking_SOA")

            'save to transaction logs
            SaveTransLogs("Create New Import Billing SOA", Me.Name, "Billing No.", Me.lblRefNo.Text, strCurrentUser)
        Else
            'EDIT BILLING
            strsql = "UPDATE bill_impsoa_main SET" &
            ...
            'save to transaction logs
            SaveTransLogs("Edit Import Billing SOA", Me.Name, "Billing No.", Me.lblRefNo.Text, strCurrentUser)
        End If

        'SAVE bill_impsoa_charges
        strsql = "DELETE FROM bill_impsoa_charges WHERE RefNo = '" & Me.lblRefNo.Text & "'"
        ...
        For intRow = 0 To Me.dtgServiceCharge.RowCount - 1
            If Len(Me.dtgServiceCharge.Rows(intRow).Cells("colSCharges").Value) > 0 Then
                strsql = "INSERT INTO bill_impsoa_charges(RefNo, Charges, Terms," &
        ...
        'SAVE bill_impsoa_acctg_entry
        strsql = "DELETE FROM bill_impsoa_acctg_entry WHERE RefNo = '" & Me.lblRefNo.Text & "'"
        ...
        DeleteToGLTable(Me.lblRefNo.Text, 16)

        Dim ctrRow As Integer

        For ctrRow = 0 To Me.dtgAcctgTitle.Rows.Count - 1
            With Me.dtgAcctgTitle.Rows(ctrRow)
                strsql = "INSERT INTO bill_impsoa_acctg_entry (RefNo, Branch, AcctCode, AcctTitle, SubAcctCode," &
                ...
                InsertToGLTable(16, GetGeneralLedgerModuleName(16), Me.lblRefNo.Text, .Cells(0).Value, .Cells(1).Value, .Cells(2).Value, .Cells(3).Value, CDbl(.Cells(9).Value), CDbl(.Cells(10).Value), .Cells(4).Value, .Cells(5).Value, .Cells(6).Value, .Cells(7).Value, .Cells(8).Value)
            End With
        Next
...
ByVal blReOpen As Boolean, ByVal blUserInput As Boolean)
    ...
    Public Sub PostRecord() Implements ICommonFunction.PostRecord
        ...
        Dim strsql As String = "UPDATE bill_impsoa_main SET" &
                               " SOANo = @SOANo, " &
                               " Posted = @Posted, " &
                               " PostedBy = @PostedBy, " &
        ...
        Try
            ...
            cmdSQL.CommandText = "UPDATE reg_trn_main SET " &
                                ...
                                "WHERE reg_group = 2 AND reg_module = 2 " &
                                    "AND reg_no = @reg_no"
    ...
    Public Sub CancelRecord() Implements ICommonFunction.CancelRecord
        ...
        If MsgBox("Are you sure you want cancel billing : " & Me.lblRefNo.Text, MsgBoxStyle.Question + MsgBoxStyle.YesNo, "System Message") = MsgBoxResult.No Then
            Exit Sub
        End If
        strsql = "UPDATE bill_impsoa_main SET Cancel=TRUE, CancelDate=@CancelDate, CancelBy='" & strCurrentUser & "' WHERE RefNo = '" & Me.lblRefNo.Text & "' AND Cancel = FALSE"
    ...
    Public Sub ReOpenRecord() Implements ICommonFunction.ReOpenRecord
        ...
        If MsgBox("Are you sure you want to re-open billing : " & Me.lblRefNo.Text, MsgBoxStyle.Question + MsgBoxStyle.YesNo, "System Message") = MsgBoxResult.No Then
            Exit Sub
        End If
        strsql = "UPDATE bill_impsoa_main SET Cancel=FALSE, CancelDate=null, CancelBy=null, Posted=FALSE, PostedDate=null, PostedBy=null WHERE RefNo = '" & Me.lblRefNo.Text & "'"
    ...
    Private Sub SOAToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SOAToolStripMenuItem.Click
        ...
        SaveTransLogs("Print Import BT S.O.A Billing", Me.Name, "Billing No.", Me.lblRefNo.Text, strCurrentUser)
        frmPrintPreview.PrintImpBSOA()
    ...
    Public Sub SearchBilling(ByVal strRefNo As String, ByVal strField As String)
        ...
        'bill_impsoa_main
        strsql = "SELECT * " &
                    "FROM bill_impsoa_main " &
                    "WHERE bill_impsoa_main.RefNo = '" & strRefNo & "'"
        ...
        'bill_impsoa_charges
        strsql = "SELECT * FROM bill_impsoa_charges WHERE RefNo = '" & strRefNo & "'"
        ...
        'bill_impb_acctg_entry
        strsql = "SELECT * FROM bill_impsoa_acctg_entry WHERE RefNo = '" & strRefNo & "'"
    ...
    Private Sub SoftCopySOAToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles SoftCopySOAToolStripMenuItem.Click
        ...
        SaveTransLogs("Print Soft Copy Import BT S.O.A Billing", Me.Name, "Billing No.", Me.lblRefNo.Text, strCurrentUser)
        frmPrintPreview.PrintImpBSOA_S()
        frmPrintPreview.ShowDialog()
    End Sub

    Private Sub DraftSOAToolStripMenuItem_Click_1(sender As Object, e As EventArgs) Handles DraftSOAToolStripMenuItem.Click
        If Len(Me.lblRefNo.Text) = 0 Then
            MsgBox("Nothing to Print!", vbInformation, "System Message")
            Exit Sub
        End If
        ...
        SaveTransLogs("Print Draft Import BT S.O.A Billing", Me.Name, "Billing No.", Me.lblRefNo.Text, strCurrentUser)
        frmPrintPreview.PrintImpBSOA_D()
        frmPrintPreview.ShowDialog()
    End Sub
End Class