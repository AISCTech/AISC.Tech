Public Class frmCreateVoucher
    ...
    Public Function GetRequestType(ByVal strReqNo As String) As String
        GetRequestType = ""
        Dim strsql As String = "SELECT REQ_ReqType FROM tbl_request WHERE REQ_Nbr = '" & strReqNo & "'"
        ...
        While reader.Read
            ...
            Select Case reader.Item("REQ_ReqType")
                Case 1
                    GetRequestType = "Petty Cash"
                Case 2
                    GetRequestType = "Check"
                Case 3
                    GetRequestType = "MC"
            End Select
        ...
        For ctrRow As Integer = 0 To Me.dtgRefNo.Rows.Count - 1
            With Me.dtgRefNo.Rows(ctrRow)
                If .Cells("colAPV").Value = True Then
                    ...
                    While reader.Read
                        frmCV.dtgParticulars.Rows.Add(reader.Item("APV_ReqNo"), .Cells("colRefNo").Value, Format(CDbl(.Cells("colAmt").Value), "n2"), .Cells("colFull").Value, Format(CDbl(.Cells("colCVAmt").Value), "n2"), strCompanyCode, GetRequestType(reader.Item("APV_ReqNo")))
                    End While

                    reader.Close()
                    cmdSQL.Dispose()
                Else
                    frmCV.dtgParticulars.Rows.Add(.Cells("colRefNo").Value, "", Format(CDbl(.Cells("colAmt").Value), "n2"), .Cells("colFull").Value, Format(CDbl(.Cells("colCVAmt").Value), "n2"), strCompanyCode, .Cells("colReqType").Value)
                End If
            End With
        Next
    End Sub
    ...
    Private Sub PopPettyCashEntry(ByVal strReqNo As String)
        ...
        Dim strsql As String = "SELECT tbl_request.PCR_Fund, tbl_pca.* FROM (ap_crp INNER JOIN tbl_pca ON ap_crp.CRP_PONbr = tbl_pca.PCA_RepNumber) " &
                                "INNER JOIN tbl_request ON tbl_pca.PCA_Nbr = tbl_request.REQ_Nbr " &
        ...
        While reader.Read
            If reader.Item("PCA_Account_Code") <> "1010" And reader.Item("PCA_DR") > 0 Then
                ...
                strFundKey = reader.Item("REQ_PCVRepFund")
        ...
        strsql = "SELECT * FROM lib_acctcode INNER JOIN lib_subacctcode ON lib_acctcode.Account_Code = lib_subacctcode.Account_Code " &
    ...
    End Sub
    Private Function CheckReqStatus(ByVal strReqNo As String) As Boolean
        If cnnDBMaster.State <> ConnectionState.Open Then cnnDBMaster.Open()
        CheckReqStatus = True
        ...
        While reader.Read()
            If reader.Item("REQ_Cancel") = True Then
                MsgBox("Request " & reader.Item("REQ_Nbr") & " already cancelled!", MsgBoxStyle.Information, "System Message")
                CheckReqStatus = False
            End If

            If reader.Item("REQ_ReqType") = 1 Then
                MsgBox("Request " & reader.Item("REQ_Nbr") & " is Petty Cash!", MsgBoxStyle.Information, "System Message")
                CheckReqStatus = False
            End If



            If frmCV.dtgParticulars.Rows.Count <> 0 Then
                Select Case frmCV.strAddNew
                    Case "Check"
                        If reader.Item("REQ_ReqType") = 3 Then
                            MsgBox("Cannot mix MC and Check Request in 1 Voucher!" & vbNewLine & "Previous request is " & frmCV.strAddNew, MsgBoxStyle.Information, "System Message")
                            CheckReqStatus = False
                        End If
                    Case "MC"
                        If reader.Item("REQ_ReqType") = 2 Then
                            MsgBox("Cannot mix MC and Check Request in 1 Voucher!" & vbNewLine & "Previous request is " & frmCV.strAddNew, MsgBoxStyle.Information, "System Message")
                            CheckReqStatus = False
                        End If
                End Select
    ...
    Private Function CheckBookingStatus(ByVal strReqNo As String) As Boolean
        If cnnDBMaster.State <> ConnectionState.Open Then cnnDBMaster.Open()
        CheckBookingStatus = True
        Dim strsql As String

        strsql = "SELECT tbl_requestd.REQD_BookNbr, tbl_requestd.REQD_Key " &
                 "FROM tbl_requestd " &
                 "WHERE tbl_requestd.REQD_REQNbr = '" & strReqNo & "'"

        Dim cmd = New MySql.Data.MySqlClient.MySqlCommand(strsql, cnnDBMaster)
        Dim reader As MySql.Data.MySqlClient.MySqlDataReader = cmd.ExecuteReader()

        While reader.Read()
            If IsDBNull(reader.Item("REQD_BookNbr")) = False Or IsDBNull(reader.Item("REQD_Key")) = False Then
                CheckBookingStatus = CheckPerBooking(reader.Item("REQD_Key"), reader.Item("REQD_Key"))
            Else
                CheckBookingStatus = True
            End If
        End While
        ...
        If CheckBookingStatus = False Then
            MsgBox("Theres a cancelled booking in request no. " & strReqNo & "!", MsgBoxStyle.Exclamation, "System Message")
        End If
    End Function
    Private Function CheckPerBooking(ByVal strBookNo As String, ByVal strKey As String) As Boolean
        If cnnDBMaster.State <> ConnectionState.Open Then cnnDBMaster.Open()
        Dim strsql As String
        Dim cmd As MySql.Data.MySqlClient.MySqlCommand
        CheckPerBooking = False
        If strKey = "Forw" Or strKey = "FORW" Then
            strsql = "SELECT BookNum, Status FROM ImpMBL WHERE Booknum = '" & strBookNo & "'"
            Dim cnn As New MySql.Data.MySqlClient.MySqlConnection(strDBMaster)

            If cnn.State <> ConnectionState.Open Then cnn.Open()

            cmd = New MySql.Data.MySqlClient.MySqlCommand(strsql, cnn)

            Dim dbReader As MySql.Data.MySqlClient.MySqlDataReader = cmd.ExecuteReader()
            While dbReader.Read
                'If Cancelled Booking
                If dbReader.Item("Status") = "3" Then
                    CheckPerBooking = False
                Else
                    CheckPerBooking = True
                End If
            End While
            dbReader.Close()
            cmd.Dispose()
        ElseIf strKey = "EXP" Then
            strsql = "SELECT StatusID " &
                        "FROM tbl_export_booking_header " &
                        "WHERE BookingNo = '" & strBookNo & "'"
            ...
            While dbReader.Read
                'If Cancelled Booking
                If dbReader.Item("StatusID") = "3" Then
                    CheckPerBooking = False
                Else
                    CheckPerBooking = True
                End If
            ...
            cmd.Dispose()
            cnn.Close()
        ElseIf strKey = "BRKG" Then
            strsql = "SELECT StatusID " &
                        "From tbl_import_booking_header " &
                        "WHERE BookingNo = '" & strBookNo & "'"
    ...
    Private Function CheckIfAccrual(ByVal strReqNo As String) As Boolean
        If cnnDBMaster.State <> ConnectionState.Open Then cnnDBMaster.Open()
        CheckIfAccrual = True
        Dim strSQL As String = "SELECT tbl_request.REQ_FTRequest, tbl_request.REQ_PCRequest " &
                 "FROM tbl_request " &
                 "WHERE tbl_request.REQ_Nbr = '" & strReqNo & "'"
        ...
        While reader.Read()
            If Me.dtgRefNo.Rows.Count = 0 Then
                If reader.Item("REQ_FTRequest") = True Then
                    Me.lblNonAccrualType.Text = "Fund Transfer"
                    Me.gpInclude.Enabled = False
                ElseIf reader.Item("REQ_PCRequest") = True Then
                    Me.lblNonAccrualType.Text = "Petty Cash"
                    Me.optAPVNo.Enabled = False
                Else
                    'Me.lblNonAccrualType.Text = "Accrual"
    ...
    Private Function SelectCRNo(ByVal strRefNo As String) As Boolean
        SelectCRNo = True
        Dim dblBal As Double = GetBalance(strRefNo, "SELECT REQ_TotalAmt FROM tbl_request WHERE REQ_Nbr = '" & strRefNo & "'", "REQ_TotalAmt")
        ...
        Dim strsql As String = "SELECT tbl_request.REQ_Nbr, tbl_request.REQ_TotalAmt, tbl_request.REQ_PayeeID, tbl_request.REQ_Cancel, tbl_request.REQ_DtNeed, tbl_vendor.Description " &
                                "FROM tbl_request INNER JOIN tbl_vendor ON tbl_request.REQ_PayeeID = tbl_vendor.Code " &
                                "WHERE tbl_request.REQ_Nbr = '" & strRefNo & "'"
        ...
        Else
            While reader.Read
                ...
                If blFull = True Then
                    If dblBal = 0 Then
                        MsgBox("Check Request No. " & reader.Item("REQ_Nbr") & " remaining amount is " & Format(CDbl(dblBal), "n2") & "!", MsgBoxStyle.Exclamation, "System Message")
                        SelectCRNo = False
                    Else
                        Me.dtgRefNo.Rows.Add(reader.Item("REQ_Nbr"), reader.Item("Description"), reader.Item("REQ_DtNeed"), Format(reader.Item("REQ_TotalAmt"), "n2"), Format(dblBal, "n2"), False, IIf(reader.Item("REQ_TotalAmt") = dblBal, blFull, False))
                    End If
                Else
                    If CDbl(Me.txtVoucherAmt.Text) > dblBal Then
                        MsgBox("Check Request No. " & reader.Item("REQ_Nbr") & " remaining amount is " & Format(CDbl(dblBal), "n2") & "!", MsgBoxStyle.Exclamation, "System Message")
                        SelectCRNo = False
                    Else
                        Me.dtgRefNo.Rows.Add(reader.Item("REQ_Nbr"), reader.Item("Description"), reader.Item("REQ_DtNeed"), Format(reader.Item("REQ_TotalAmt"), "n2"), Format(CDbl(Me.txtVoucherAmt.Text), "n2"), False, blFull)
                    End If
                End If
            End While
        End If
    ...
    Private Function SelectAPVCNo(ByVal strRefNo As String) As Boolean
        SelectAPVCNo = True
        Dim blFull As Boolean, strReqNo As String = "", acctcode As String = ""
        ...
        Dim strsql As String = "SELECT tbl_apv.APV_Nbr, tbl_apv.APV_Cancel, tbl_apvgl.APVGL_CR, tbl_apvgl.APVGL_Account_Code, " &
                                "tbl_request.REQ_Nbr, tbl_request.REQ_PayeeID, tbl_request.REQ_DtNeed, tbl_vendor.Description " &
                                "FROM (((tbl_apv INNER JOIN tbl_apvgl ON tbl_apv.APV_Nbr = tbl_apvgl.APVGL_APVNbr) " &
                                "INNER JOIN tbl_request ON tbl_apv.APV_ReqNo = tbl_request.REQ_Nbr) " &
                                "INNER JOIN lib_vendor ON tbl_request.REQ_PayeeID =tbl_vendor.Code) " &
                                "WHERE tbl_apv.APV_Nbr = '" & strRefNo & "' " &
                                "AND (tbl_apvgl.APVGL_Account_Code = '2010' OR tbl_apvgl.APVGL_Account_Code = '6250') " &
                                "AND tbl_apvgl.APVGL_CR > 0"
        ...
        Else
            ...
            If CheckReqStatus(strReqNo) = False Then Exit Function
            If CheckBookingStatus(strReqNo) = False Then Exit Function
            ...
            While reader.Read
                ...
                If blFull = True Then
                    If dblBal = 0 Then
                        ...
                        SelectAPVCNo = False
                    Else
                        Me.dtgRefNo.Rows.Add(reader.Item("APV_Nbr"), reader.Item("Description"), reader.Item("REQ_DtNeed"), Format(reader.Item("APVGL_CR"), "n2"), Format(dblBal, "n2"), True, IIf(reader.Item("APVGL_CR") = dblBal, blFull, False))
                    End If
                Else
                    If CDbl(Me.txtVoucherAmt.Text) > dblBal Then
                        MsgBox("AP Voucher No. " & reader.Item("APV_Nbr") & " remaining amount is " & Format(CDbl(dblBal), "n2") & "!", MsgBoxStyle.Exclamation, "System Message")
                        SelectAPVCNo = False
                    Else
                        Me.dtgRefNo.Rows.Add(reader.Item("APV_Nbr"), reader.Item("Description"), reader.Item("REQ_DtNeed"), Format(reader.Item("APVGL_CR"), "n2"), Format(CDbl(Me.txtVoucherAmt.Text), "n2"), True, blFull)
                    End If
                End If
            End While
        End If
    ...
    Private Sub cmdSelect_Click(sender As Object, e As EventArgs) Handles cmdSelect.Click
        ...
        '    Case "CV"
        If Me.optReqNo.Checked = True Then
            If CheckReqStatus(Me.lblPrefix.Text & Me.txtRefNo.Text) = False Then Exit Sub
            If CheckBookingStatus(Me.lblPrefix.Text & Me.txtRefNo.Text) = False Then Exit Sub
            If CheckIfRequestHasAP(Me.lblPrefix.Text & Me.txtRefNo.Text) = True Then Exit Sub
            If CheckIfAccrual(Me.lblPrefix.Text & Me.txtRefNo.Text) = False Then Exit Sub
            If SelectCRNo(Me.lblPrefix.Text & Me.txtRefNo.Text) = False Then Exit Sub
